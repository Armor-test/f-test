describe('hx-disinherit attribute', function() {
  ...
  it('basic inheritance sanity-check', function() {
    const response_inner = '<div id="snowflake" class="">Hello world</div>'
    const response = '<div id="unique" class="">' + response_inner + '</div>'
    this.server.respondWith('GET', '/test', response)

    const div = make('<div hx-select="#snowflake" hx-target="#cta" hx-swap="outerHTML"><button id="bx1" hx-get="/test"><span id="cta">Click Me!</span></button></div>')
    const btn = byId('bx1')
    btn.click()
    this.server.respond()
  ...
  it('disinherit exclude single attribute', function() {
    const response_inner = '<div id="snowflake">Hello world</div>'
    const response = '<div id="unique">' + response_inner + '</div>'
    this.server.respondWith('GET', '/test', response)

    const div = make('<div hx-select="#snowflake" hx-target="#cta" hx-swap="beforebegin" hx-disinherit="hx-select"><button id="bx1" hx-get="/test"><span id="cta">Click Me!</span></button></div>')
    const btn = byId('bx1')
    btn.click()
    this.server.respond()
  ...
  it('disinherit exclude multiple attributes', function() {
    ...
    const div = make('<div hx-select="#snowflake" hx-target="#cta" hx-swap="beforebegin" hx-disinherit="hx-select hx-swap">' +
  ...
  it('disinherit exclude all attributes', function() {
    ...
    const response = '<div id="unique">' + response_inner + '</div>'
    this.server.respondWith('GET', '/test', response)
    const div = make('<div hx-select="#snowflake" hx-target="#cta" hx-swap="beforebegin" hx-disinherit="*">' +
  ...
  it('same-element inheritance disable', function() {
    ...
    const btn = make('<button hx-select="#snowflake" hx-target="#container" hx-trigger="click" hx-get="/test" hx-swap="outerHTML" hx-disinherit="*"><div id="container"></div></button>')
    btn.click()
    this.server.respond()
    btn.firstChild.id.should.equal('snowflake')
  ...
  it('same-element inheritance disable with child nodes', function() {
    ...
    const div = make('<div hx-select="#snowflake" hx-target="#container" hx-get="/test" hx-swap="outerHTML" hx-trigger="keyup" hx-disinherit="*"><div id="container"><button id="bx1" hx-get="/test2" hx-trigger="click" hx-target="#target"><div id="target"></div></button></div></div>')
    const btn = byId('bx1')
    btn.click()
    this.server.respond()
    btn.firstChild.id.should.equal('target')
    btn.firstChild.innerText.should.equal('unique-snowflake')
    const count = (div.parentElement.innerHTML.match(/snowflake/g) || []).length
  ...
  it('boosted element hx-disinherit sanity check', function() {
    try {
      const request
      const handler = htmx.on('htmx:beforeRequest', function(evt) {
        request = evt
      })
      const div = make('<div hx-boost="true" hx-disinherit="false"><a id="a1" href="/test">Click me</a></div>')
      const link = byId('a1')
      link.click()
  ...
  it('boosted element inheritance manual unset', function() {
    try {
      ...
      const div = make('<div hx-boost="true" hx-get="/test"><div hx-boost="unset"><a id="a1" href="/test">Click me</a></div></div>')
      const link = byId('a1')
  ...
  it('nested htmx-node with boosting parent', function() {
    try {
      ...
      const div = make('<div hx-boost="true" hx-target="#test" hx-disinherit="*"><div id="test"></div><a id="a1" href="/test" hx-get="/test2">Click me</a></div>')
      const link = byId('a1')
      link.click()
